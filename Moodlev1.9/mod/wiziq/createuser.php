...
$obj->password=$pass;
$obj->TimeZone=$time;
$obj->M_InstallationID=$fqdn;
$obj->M_UserID=$id;
$obj->M_EmailAddress=$email1;
$obj->M_UserName=$usr;
$obj->TimeZoneDesc=$timezone;
...
$err = $client->getError();
if ($err) {
	$error="The service is temporarily unavailable. We are sorry for the inconvenience. Please try after sometime.";
}


$result = $client->call('CreateMoodleUser',array('oMoodleRegister'=>$obj),$namespace='http://tempuri.org',$soapAction='http://tempuri.org/CreateMoodleUser',$headers=false,$rpcParams=null,$style='rpc',$use='encoded');
...
if ($client->fault) {
 $error="A user with this email address already exists on WiZiQ. If it’s you please sign in.";
}
require_login();
...
//            $pageheading = get_string("createmoodleuser", "moodle", $fullmodulename);
  $pageheading="Schedule Your Session in WIZQ";
        }
        $strnav = '';

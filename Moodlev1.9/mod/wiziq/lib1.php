<?php // $Id: lib.php,v 1.164.2.25 2007/10/19 02:27:27 toyomoyo Exp $
...
function calendar_sql_where($tstart, $tend, $users, $groups, $courses, $withduration=true, $ignorehidden=true) {
    ...
    if(is_array($users) && !empty($users)) {
        ...
        $whereclause .= ' (userid IN ('.implode(',', $users).') AND courseid = 0 AND groupid = 0 AND modulename="wiziq")';
    }
    else if(is_numeric($users)) {
        // Events from one user
        if(!empty($whereclause)) $whereclause .= ' OR';
        $whereclause .= ' (userid = '.$users.' AND courseid = 0 AND groupid = 0 AND modulename="wiziq")';
    }
    else if($users === true) {
        // Events from ALL users
        if(!empty($whereclause)) $whereclause .= ' OR';
        $whereclause .= ' (userid != 0 AND courseid = 0 AND groupid = 0 AND modulename="wiziq")';
    }
    else if($users === false) {

...
class detail {
      ...
      var $EmailAddress;
		...
		var $MoodleEmailAdress;
			...
			$obj->Password=$pass;
			$obj->MoodleEmailAdress=$memail;
			
//	$obj->EmailAddress="nitin.goswami@mechante.com";
...
$result = $client->call('MoodleManualLogin',array('oMoodleManualLogin'=>$obj),$namespace='http://tempuri.org',$soapAction='http://tempuri.org/MoodleManualLogin',$headers=false,$rpcParams=null,$style='rpc',$use='encoded');
...
//            $pageheading = get_string("createmoodleuser", "moodle", $fullmodulename);
  $pageheading="Schedule Your WiZiQ Live Class";
        }
        $strnav = '';
...
{
$modform1=$CFG->dirroot."/mod/$module->name/sign-in.htm";
$pageheading="Sign in on WiZiQ”";
  print_heading_with_help($pageheading, "signin_help", $module->name, $icon);
        print_simple_box_start('center', '', '', 5, 'generalbox', $module->name);
   echo '<strong><center><font color="red">Email address and Password do not match. Please try again.</font></center></strong>';
   include_once($modform1);
   //error("Email address and Password do not match. Please try again.");
print_simple_box_end();
